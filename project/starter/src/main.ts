...
async function main() {
  ...
  fs.writeFileSync(path.join(reportsDir, `${baseName}.json`), jsonReport, 'utf-8');

  fs.writeFileSync(path.join(reportsDir, `${baseName}.md`), markdownReport, 'utf-8');
  
  fs.writeFileSync(path.join(reportsDir, `${baseName}.html`), htmlReport, 'utf-8' );

  // Canonical report filenames
  fs.writeFileSync(path.join(reportsDir, 'report.json'), jsonReport, 'utf-8');
  fs.writeFileSync(path.join(reportsDir, 'report.md'), markdownReport, 'utf-8');
  fs.writeFileSync(path.join(reportsDir, 'report.html'), htmlReport, 'utf-8');
